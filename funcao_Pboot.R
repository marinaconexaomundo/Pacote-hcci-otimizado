function (model, significance = 0.05, double = FALSE, J = NULL, 
          K = NULL, distribution = "rademacher") 
{
  if (class(model) != "lm") 
  ...
  if (is.null(J) == TRUE || is.null(K) == TRUE) {
    ...
    gamma2 = ((1/2) * significance^(-2) * (1 - significance) * 
                (significance + 1/4))^(1/3)
    J = as.integer(gamma2 * L^(2/3))
    K = as.integer(gamma2^(-1) * L^(1/3))
  }
  is.wholenumber <- function(x, tol = .Machine$double.eps^0.5) abs(x - 
                                                                     round(x)) < tol
  while (is.wholenumber(K/2) == FALSE && is.wholenumber((J + 
                                                         1) * significance) == FALSE) {
  ...
  for (j in 1:J) {
    ...
    if (double == TRUE) {
      for (k in 1:K) {
        ...
        else t_star_star = rnorm(length(model$fitted.values), 
                                 0, 1)
        y_star_star = as.vector(Xbeta_star + t_star_star * 
                                  error_hat_star/root_1_less_h)
        model_string = as.character(model$call$formula)
        ...
        for (m in 1:number_parameters) {
          if (beta_star_star[m] <= 2 * beta_star[m] - 
              beta[m]) {
            matrix_u[k, m] = 1
          }
          else matrix_u[k, m] = 0
  ...
  for (m in 1:number_parameters) {
    ic_inf_simple[m] = quantile(as.vector(matrix_beta_star[, 
                                                           m]), significance/2)
    ic_sup_simple[m] = quantile(as.vector(matrix_beta_star[, 
                                                           m]), 1 - significance/2)
    if (double == TRUE) {
      ic_inf_double[m] = quantile(as.vector(matrix_beta_star[, 
                                                             m]), quantile(as.vector(U[, m]), significance/2))
      ic_sup_double[m] = quantile(as.vector(matrix_beta_star[, 
                                                             m]), quantile(as.vector(U[, m]), 1 - significance/2))
    }
  }
